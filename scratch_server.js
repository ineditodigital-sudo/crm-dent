...
// M├âãÆ├é┬│dulos con carga opcional
let google, pino;
...
console.log('­ƒôØ Configuraci├│n cargada. DB_HOST:', process.env.DB_HOST || 'No definido', 'GEMINI_KEY present:', !!process.env.GEMINI_API_KEY);
...
console.log('­ƒôé Directorio de ejecuci├│n (CWD):', process.cwd());
const distDir = '/home/inedito/public_html/crm-dent.inedito.digital';
const uploadDir = path.join(distDir, 'uploads');
console.log('­ƒôé Carpeta dist detectada en:', distDir);
console.log('­ƒôé Carpeta de uploads configurada en:', uploadDir);
try {
    if (!fs.existsSync(uploadDir)) {
        console.log('­ƒôé Directorio de uploads no existe. Creando en:', uploadDir);
        fs.mkdirSync(uploadDir, { recursive: true, mode: 0o755 });
    } else {
        console.log('­ƒôé Directorio de uploads detectado en:', uploadDir);
    }
    ...
    if (!fs.existsSync(htaccessPath)) {
        ...
        console.log('Ô£à Archivo .htaccess creado en uploads para acceso directo');
    }
} catch (err) {
    console.error('ÔØî Error configurando directorio de uploads:', err.message);
}
...
app.use('/uploads', (req, res) => {
    console.error(`ÔÜá´©Å Archivo no encontrado en uploads: ${req.url} (Buscando en: ${path.join(uploadDir, req.url)})`);
    res.status(404).send('Archivo no encontrado');
});
if (fs.existsSync(distDir)) app.use(express.static(distDir));

// Helper: llama a Gemini con retry autom├ítico y fallback de modelos (v├¡a Fetch directo a V1)
async function callGemini(genAI, prompt, retries = 2) {
    const apiKey = genAI.apiKey || process.env.GEMINI_API_KEY;
    if (!apiKey) throw new Error('No se encontr├│ API Key para Gemini');
    ...
    for (const modelName of modelsToTry) {
        try {
            console.log(`­ƒñû Intentando (Direct v1) con modelo: ${modelName}...`);
            ...
            for (let i = 0; i < retries; i++) {
                try {
                    ...
                    if (!data.candidates?.[0]?.content?.parts?.[0]?.text) {
                        throw new Error('Respuesta de IA vac├¡a o malformada');
                    }

                    const text = data.candidates[0].content.parts[0].text;
                    console.log(`Ô£à ├ëxito con modelo: ${modelName}`);
                    return text;
                } catch (err) {
                    if (err.message && err.message.includes('429') && i < retries - 1) {
                        const wait = (i + 1) * 10000;
                        console.log(`ÔÅ│ Rate limit (429) en ${modelName}, reintentando en ${wait / 1000}s...`);
            ...
            lastError = err;
            console.error(`ÔÜá´©Å Fall├│ modelo ${modelName}:`, err.message);
...
async function initDB() {
    try {
        console.log('├â┬░├à┬©├óÔé¼┼ô├é┬ª Inicializando base de datos...');
        ...
        // Limpiar duplicados antes de poner la clave ├âãÆ├é┬║nica
        try {
            await db.execute(`
            ...
            console.log('├â┬ó├àÔÇ£├óÔé¼┬ª Clave ├âãÆ├é┬║nica a├âãÆ├é┬▒adida a crm_settings');
        } catch (e) {
            console.log('├â┬ó├óÔé¼┼¥├é┬╣├â┬»├é┬©├é┬Å Nota: La clave ├âãÆ├é┬║nica ya exist├âãÆ├é┬¡a o no se pudo crear (pero se intent├âãÆ├é┬│ limpiar)');
        }
        ...
        // --- MIGRACIONES AUTOM├âãÆ├é┬üTICAS (para tablas ya existentes) ---
        const migrations = [
        ...
        console.log('├â┬ó├àÔÇ£├óÔé¼┬ª Base de datos lista y migrada.');
...
app.post('/api/settings/:section?', requireAuth, async (req, res) => {
    try {
        ...
        if (!section) return res.status(400).json({ error: 'Secci├│n no especificada' });

        console.log(`­ƒÆ¥ Guardando configuraci├│n para secci├│n: ${section}`, Object.keys(data));
        ...
        console.error('ÔØî Error al guardar settings:', err.message);
...
app.post('/api/conversations/:id/send', requireAuth, async (req, res) => {
    ...
    try {
        ...
        console.error('├â┬ó├é┬Ø├àÔÇÖ Error enviando mensaje:', err.message);
        res.status(500).json({ error: err.message });
    }
});
...
app.patch('/api/patients/:id/status', requireAuth, async (req, res) => {
    ...
    if (!allowed.includes(status)) return res.status(400).json({ error: 'Estado no v├âãÆ├é┬ílido' });
    try {
...
app.patch('/api/appointments/:id/cancel', requireAuth, async (req, res) => {
    try {
        ...
        if (rows[0]?.google_event_id) {
            ...
            } catch (e) { console.error('├â┬ó├à┬í├é┬á├â┬»├é┬©├é┬Å No se pudo borrar de GCal:', e.message); }
        }
...
app.post('/api/brand/upload', requireAuth, (req, res, next) => {
    upload.single('image')(req, res, (err) => {
        if (err) {
            console.error('ÔØî Error de Multer:', err.message);
            return res.status(500).json({ error: 'Error en el servidor al procesar la imagen: ' + err.message });
        }
        if (!req.file) {
            console.error('ÔØî No se recibi├│ ning├║n archivo en req.file');
            return res.status(400).json({ error: 'No se subi├│ ning├║n archivo' });
        }
        console.log('Ô£à Archivo subido con ├®xito:', req.file.filename);
        console.log('­ƒôì Guardado en:', path.join(uploadDir, req.file.filename));
        ...
        try {
            ...
            if (fs.existsSync(req.file.path)) {
                console.log('Ô£à Archivo verificado f├¡sicamente en el disco:', req.file.path);
            } else {
                console.error('ÔØî El archivo NO se encuentra donde deber├¡a tras guardarse:', req.file.path);
            }
        } catch (chmodErr) {
            console.error('ÔÜá´©Å No se pudieron aplicar permisos 644:', chmodErr.message);
        }
...
// SSE ├â┬ó├óÔÇÜ┬¼├óÔé¼┬Ø Notificaciones en tiempo real
...
app.get('/api/reports/ai', requireAuth, async (req, res) => {
    ...
    try {
        ...
        // Recopilar datos b├âãÆ├é┬ísicos para que la IA los analice
        const [[t]] = await db.execute(`
            SELECT 
                (SELECT COUNT(*) FROM patients) as total_patients,
        ...
        const prompt = `Act├âãÆ├é┬║a como un consultor de negocios y analiza estas m├âãÆ├é┬®tricas de una cl├âãÆ├é┬¡nica:
- Total de pacientes registrados: ${t.total_patients}
- Citas generadas autom├âãÆ├é┬íticamente por el bot: ${t.bot_appointments}

Escribe un reporte MUY BREVE (m├âãÆ├é┬íximo 3 p├âãÆ├é┬írrafos cortos) con:
1. Una observaci├âãÆ├é┬│n sobre el desempe├âãÆ├é┬▒o actual.
2. Una recomendaci├âãÆ├é┬│n clara para mejorar la retenci├âãÆ├é┬│n o las ventas.
3. Un tono motivador y profesional.
Escribe en texto plano, sin negritas ni markdown, usando vi├âãÆ├é┬▒etas simples (├â┬ó├óÔÇÜ┬¼├é┬ó) si es necesario.`;
...
app.get('/api/reports/ai', requireAuth, async (req, res) => {
    try {
        ...
        if (keys.gemini && GoogleGenerativeAI) {
            const genAI = new GoogleGenerativeAI(keys.gemini, { apiVersion: 'v1' });
            const prompt = `Analiza estas m├âãÆ├é┬®tricas de una cl├âãÆ├é┬¡nica dental y da un resumen estrat├âãÆ├é┬®gico breve (3 p├âãÆ├é┬írrafos):
            ...
            Enf├âãÆ├é┬│cate en la eficiencia del bot y sugerencias para mejorar la conversi├âãÆ├é┬│n.`;
...
async function inicializarLibreriasYWhatsApp() {
    ...
    } catch (e) { console.error('Error librer├âãÆ├é┬¡as:', e.message); }
}
    ...
    try {
        ...
        socket.ev.on('connection.update', (u) => {
            ...
            if (u.connection === 'open') {
                lastQR = "";
                lastError = null;
                console.log('├â┬ó├àÔÇ£├óÔé¼┬ª WhatsApp conectado');
            }
            if (u.connection === 'close') {
                const code = u.lastDisconnect?.error?.output?.statusCode;
                console.log(`├â┬░├à┬©├óÔé¼┬Ø├àÔÇÖ Conexi├âãÆ├é┬│n cerrada. C├âãÆ├é┬│digo: ${code}`);

                if (code === DisconnectReason.loggedOut) {
                    console.log('├â┬░├à┬©├à┬í├é┬¬ Sesi├âãÆ├é┬│n cerrada por el usuario. Limpiando datos...');
                    lastQR = "";
                    socket = null;
                    ...
                    if (fs.existsSync('auth_info_baileys')) {
                        ...
                        } catch (e) { console.error('Error al borrar sesi├âãÆ├é┬│n:', e.message); }
                    }
        ...
        socket.ev.on('messages.upsert', async ({ messages }) => {
            ...
            console.log(`­ƒôÑ RECIBIDO de ${sender}: ${text}`);
            ...
            if (keys.gemini && GoogleGenerativeAI && pManual !== 1) {
                try {
                    console.log(`­ƒñû Generando respuesta para ${sender} con Gemini...`);
                    ...
                    const personality = brandCfg.bot_personality || brandCfg.bot_context ||
                        `Eres el asistente virtual de ${brandCfg.clinic_name || 'nuestra cl├â┬¡nica'}. Eres amable y profesional. Responde siempre en espa├â┬▒ol.`;
...
REGLAS CR├â TICAS DE AGENDADO:
1. ANTES de agendar, DEBES tener: Nombre, Email y Tel├â┬®fono (10 d├â┬¡gitos).
2. Si falta algo, p├â┬¡delo amablemente. Datos actuales: Nombre: ${patientInfo.name || 'Desconocido'}, Email: ${patientInfo.email || 'Desconocido'}, Tel: ${patientInfo.phone}.
3. Si el paciente da sus datos, usa: __DATOS|nombre=X|email=Y|telefono=Z__
4. S├âÔÇ£LO cuando tengas TODO, usa: __CITA|fecha=YYYY-MM-DD HH:mm:ss|servicio=NombreServicio__
5. El horario de atenci├â┬│n es Lunes a S├â┬íbado de 9:00 AM a 7:00 PM. No agendes fuera de este horario ni en el pasado.

SERVICIOS DISPONIBLES:
${servicesList}

HISTORIAL DE CONVERSACI├âÔÇ£N:
${conversationHistory}
...
Asistente:`;
                    ...
                    if (citaMatch) {
                        ...
                        if (isNaN(fechaCita.getTime())) {
                            botMsg = "Lo siento, la fecha no es v├â┬ílida. ├é┬┐Podr├â┬¡as decirme otra vez cu├â┬índo te gustar├â┬¡a venir?";
                        } else if (fechaCita < now) {
                            botMsg = "Lo siento, no puedo agendar citas en el pasado. ├é┬┐Podr├â┬¡as elegir otra fecha y hora?";
                        } else if (day === 0 || hour < 9 || hour >= 19) {
                            botMsg = "Nuestro horario de atenci├â┬│n es de Lunes a S├â┬íbado de 9:00 AM a 7:00 PM. ├é┬┐Gustas elegir otro horario?";
                        } else {
                            ...
                            if (overlaps.length > 0) {
                                botMsg = "Ese horario ya est├â┬í ocupado. ├é┬┐Tendr├â┬ís alg├â┬║n otro momento disponible?";
                            } else {
                                ...
                                try {
                                    ...
                                    if (cal) {
                                        const event = {
                                            ...
                                            description: `Agendada autom├â┬íticamente por el Asistente de WhatsApp.`,
                                ...
                                if (gUrl) botMsg += `\n\n­ƒôà *┬íCita confirmada!* Puedes guardarla en tu calendario aqu├â┬¡: ${gUrl}`;
                    ...
                    if (botMsg.trim()) {
                        ...
                        console.log(`Ô£à Respuesta enviada a ${sender}`);
                    }
                } catch (errIA) {
                    console.error('ÔØî Error flujo Gemini:', errIA.message);
                }
            }
        });
    } catch (e) {
        console.error('ÔØî Error Socket:', e.message);
    }
}
...
app.post('/api/whatsapp/restart', requireAuth, async (req, res) => {
    try {
        console.log('├â┬░├à┬©├óÔé¼┬Ø├óÔé¼┼¥ Reiniciando conexi├âãÆ├é┬│n de WhatsApp...');
        if (socket) {
...
app.post('/api/whatsapp/logout', requireAuth, async (req, res) => {
    try {
        console.log('├â┬░├à┬©├à┬í├é┬¬ Forzando cierre de sesi├âãÆ├é┬│n...');
        if (socket) {
    ...
    try {
        ...
        for (const appt of appointments) {
            ...
            try {
                ...
                const msg = `├â┬░├à┬©├óÔé¼┼ô├àÔÇÖ *Recordatorio de Cita*\n\nHola ${appt.patient_name || 'estimado paciente'}, te recordamos que ma├âãÆ├é┬▒ana *${fechaStr} a las ${horaStr}* tienes una cita agendada.\n\n├âÔÇÜ├é┬┐Alguna duda o necesitas reprogramarla? Resp├âãÆ├é┬│ndenos aqu├âãÆ├é┬¡.`;
                ...
                console.log(`├â┬░├à┬©├óÔé¼ÔÇØ├óÔé¼┼ô├â┬»├é┬©├é┬Å Recordatorio enviado a ${appt.patient_name} (${appt.phone})`);
            } catch (e) {
                console.error(`├â┬ó├é┬Ø├àÔÇÖ Error enviando recordatorio a ${appt.phone}:`, e.message);
            }
        }
    } catch (e) {
        console.error('├â┬ó├é┬Ø├àÔÇÖ Error en cron de recordatorios:', e.message);
    }
}
